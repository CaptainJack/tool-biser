...
object DataPairs {
	...
	val int = listOf(
		...
		(-128)
			.asBytes(0xFE, 0xFF, 0xFF, 0xFF, 0x80),
		(-127)
			.asBytes(0xFE, 0xFF, 0xFF, 0xFF, 0x81),
		(-1)
			.asBytes(0xFF),
		0
			.asBytes(0x00),
		...
		253
			.asBytes(0xFD),
		254
			.asBytes(0xFE, 0x00, 0x00, 0x00, 0xFE),
		255
			.asBytes(0xFE, 0x00, 0x00, 0x00, 0xFF),
		16_843_260
			.asBytes(0xFE, 0x01, 0x01, 0x01, 0xFC),
		Int.MAX_VALUE
	...
	val long = listOf(
		...
		(-128L)
			.asBytes(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x80),
		(-127L)
			.asBytes(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x81),
		(-1L)
			.asBytes(0xFF),
		0L
			.asBytes(0x00),
		...
		253L
			.asBytes(0xFD),
		254L
			.asBytes(0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFE),
		255L
			.asBytes(0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF),
		16_843_260L
			.asBytes(0xFE, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0xFC),
		2147483647L
	...
	val shortArray = listOf(
		shortArrayOf()
			.asBytes(0x00),
		short.map { it.first }.toShortArray()
			to listOf(short.size.toByte()).plus(short.flatMap { it.second.toList() }).toByteArray()
	)
	...
	val listString = listOf(
		listOf<String>()
			.asBytes(0x00),
		listOf(null, "A", "B")
			.asBytes(0x03, 0xFF, 0x01, 0x41, 0x01, 0x42)
	)
